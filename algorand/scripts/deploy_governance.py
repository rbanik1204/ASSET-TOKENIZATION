...
def deploy_contract(network: str, creator_mnemonic: str, asa_id: int):
    ...
    try:
        ...
        contract_path = os.path.join(current_dir, '..', 'contracts', 'governance.py')
        ...
        # Define state schema
        # Global: admin (bytes), asa_id (uint), proposal_count (uint), quorum (uint), paused (uint)
...
def main():
    parser = argparse.ArgumentParser(description='Deploy Governance Smart Contract')
